...
Ext.onReady(function() {
	...
	var main_grid = new Ext.grid.GridPanel({
		...
		update: function(Param) {
			...
			Ext.Ajax.request({
				url: URLS.base + 'panel/content/tag/action',
	...
	function main_win(param) {
		var win = new Ext.Window({
			...
			listeners: {
				show: function(w) {
					var Title = (param.id == 0) ? 'Entry Tag - [New]' : 'Entry Tag - [Edit]';
					w.setTitle(Title);
					
					Ext.Ajax.request({
						url: URLS.base + 'panel/content/tag/view',
						success: function(Result) {
			...
			save: function() {
				...
				Func.ajax({ param: ajax, url: URLS.base + 'panel/content/tag/action', callback: function(result) {
					Ext.Msg.alert('Informasi', result.message);
					if (result.status) {
						main_store.load();
						win.hide();
