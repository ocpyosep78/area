...
Ext.onReady(function() {
	...
	var main_store = Ext.create('Ext.data.Store', {
		...
		proxy: {
			type: 'ajax',
			url : URLS.base + 'panel/adsense/adsense_owner/grid', actionMethods: { read: 'POST' },
	...
	var main_grid = new Ext.grid.GridPanel({
		...
		columns: [ {
			...
			}, {	header: 'Priority', dataIndex: 'priority', sortable: true, filter: true, width: 200
		} ],
		tbar: [ {
		...
		update: function(Param) {
			...
			Ext.Ajax.request({
				url: URLS.base + 'panel/adsense/adsense_owner/action',
	...
	function main_win(param) {
		var win = new Ext.Window({
			...
			listeners: {
				show: function(w) {
					var Title = (param.id == 0) ? 'Entry Owner - [New]' : 'Entry Owner - [Edit]';
					w.setTitle(Title);
					
					Ext.Ajax.request({
						url: URLS.base + 'panel/adsense/adsense_owner/view',
						success: function(Result) {
							...
							win.priority = new Ext.form.TextField({ renderTo: 'priorityED', width: 225 });
							
							// Populate Record
							if (param.id > 0) {
								win.name.setValue(param.name);
								win.priority.setValue(param.priority);
							}
						}
					});
				},
			...
			save: function() {
				...
				ajax.priority = win.priority.getValue();
				...
				Func.ajax({ param: ajax, url: URLS.base + 'panel/adsense/adsense_owner/action', callback: function(result) {
					Ext.Msg.alert('Informasi', result.message);
					if (result.status) {
						main_store.load();
						win.hide();
