...
class movie76 {
	...
	function get_array_clear($content) {
		...
		// remove start offset
		$offset = '<!-- #utility-before-content .utility -->';
		$pos_first = strpos($content, $offset);
		$content = substr($content, $pos_first, strlen($content) - $pos_first);
		
		// remove end offset
		$offset = '<div class="navigation-links">';
		$pos_end = strpos($content, $offset);
		$content = substr($content, 0, $pos_end);
		
		preg_match_all("/<h2 class='post-title entry-title'><a href='([^']+)'>([^<]+)<\/a>/i", $content, $match);
		foreach ($match[0] as $key => $value) {
			if (!empty($match[1][$key]) && $match[2][$key]) {
				$link = $match[1][$key];
				$title = preg_replace('/^film\s*/i', '', $match[2][$key]);
				$title = preg_replace('/[^\x20-\x7E|\x0A]/i', '', $title);
				
				$array_temp = array( 'link' => $link, 'title' => $title );
	...
	function get_desc($content) {
		...
		// endfix
		if (!empty($result)) {
			$result .= '<br /><br /><div>Sumber : Movie76</div>';
		}
	...
	function get_image($content) {
		...
		// get link image
		preg_match('/class="entry-content">\s*(<div>\s*)*<img src="([^"]+)" \//i', $content, $match);
