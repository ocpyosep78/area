...
class downloadfilem {
	...
	function get_content($link_source) {
		...
		// remove wording
		$content = preg_replace('/<a href="[^"]+">\s*(<b>)*Download\/internet[^<]+(<\/b>)*<\/a>/i', '', $content);
		
		// remove additional
		$offset = 'Download MF.php';
		$pos_end = strpos($content, $offset);
		if ($pos_end) {
			$content = substr($content, 0, $pos_end);
		}
	...
	function get_desc($content) {
		...
		// endfix
		if (!empty($result)) {
			$result .= '<br /><br /><div>Sumber : Download Filem</div>';
		}
	...
	function get_image($content) {
		...
		// get link image
		preg_match('/src="(http:\/\/www.downloadfilem.com\/wp-content\/uploads\/\d+\/\d+[^"]+)"/i', $content, $match);
